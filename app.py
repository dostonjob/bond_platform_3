...
# /* ── Transaction pills ── */
# .tx-buy{background:#EBF3FB;border-left:4px solid #2E75B6;border-radius:6px;
#   padding:.55rem .9rem;margin:.35rem 0;font-size:.83rem;line-height:1.5;}
# .tx-sell-partial{background:#FFF8E1;border-left:4px solid #E8A000;border-radius:6px;
#   padding:.55rem .9rem;margin:.35rem 0;font-size:.83rem;line-height:1.5;}
# .tx-sell-full{background:#FDECEA;border-left:4px solid #C00000;border-radius:6px;
#   padding:.55rem .9rem;margin:.35rem 0;font-size:.83rem;line-height:1.5;}
...
#     derive_accrued_interest, fmt_date, parse_date
# )
...
#             'Price (WAC)':     r.get('price'),
#             'MTM':             r.get('mtm'),
#             'OCI G/L':         r.get('oci_gl'),
#             'NAV':             r.get('nav'),
#             'Realized P&L':    r.get('realized_pl'),
#         })
...
#             with c1: tx_acc = st.number_input("Accrued Interest at this transaction", value=0.0, format="%.4f")
...
#         c1,_ = st.columns([2,5])
#         with c1: nt_acc = st.number_input("Accrued Interest", value=0.0, format="%.4f")
...
#         ("Realized P&L",          f"{summary['total_realized_pl']:+,.2f}",
...
#     st.dataframe(
#         df.style.format({c:'{:,.4f}' for c in num_cols}, na_rep='—'),
...
#                 'Nominal_Balance':r.get('nominal_balance'),'Price_WAC':r.get('price'),
#                 'MTM':r.get('mtm'),'OCI_GL':r.get('oci_gl'),'NAV':r.get('nav'),
#                 'Realized_PL':r.get('realized_pl')})
